...
(function initFadeScenes() {
  ...
  // On snap-scroll, scenes snap fully into view — trigger at very low threshold
  const observer = new IntersectionObserver(
    (entries) => {
      entries.forEach(entry => {
        if (entry.isIntersecting) {
          // Small delay so snap animation settles before fade starts
    ...
    (entries) => {
      if (entries[0].isIntersecting && !triggered) {
        ...
        lines.forEach((line, i) => {
          // 80ms settle delay + staggered lines
          setTimeout(() => {
